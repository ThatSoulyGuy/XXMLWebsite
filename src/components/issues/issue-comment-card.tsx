...
import { editIssueComment, deleteIssueComment } from "@/actions/issues";
...
interface IssueCommentRevision {
  id: string;
  body: string;
  createdAt: Date;
  editor: {
...
export function IssueCommentCard({ comment, canEdit, canDelete }: IssueCommentCardProps) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    setError(null);
    const result = await editIssueComment(comment.id, editedBody);
    if (result.error) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    const result = await deleteIssueComment(comment.id);
    if (result.error) {
      alert(result.error);
    }
    setIsDeleting(false);
